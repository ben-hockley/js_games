...
// Generates a new board and fills the cells on the soduku table.
// Difficulty may be a minimum of 1(easiest) and a maximum of 57(hardest)
function fill_new_board() {
    console.log("Generating New Board...")

    // get the difficulty value from the difficulty input field
    difficulty = parseInt(document.getElementById("difficulty-input").value) || 30;

    board = newStartingBoard(holes = difficulty)

    starting_board = board[1] // 9 x 9 matrix containing the starting board (0s are empty cells)
    finished_board = board[2] // 9 x 9 matrix containing the solution.

    document.querySelectorAll('.sudoku-cell').forEach((cell, index) => {
        cell.classList.remove('bg-success', 'bg-danger');
        cell_value = starting_board[Math.floor(index / 9)][index % 9];
        if (cell_value !== 0) {
...
function solve_board() {
    ...
    document.querySelectorAll('.sudoku-cell').forEach((cell, index) => {
        cell.classList.remove('bg-success', 'bg-danger');
        cell_value = finished_board[Math.floor(index / 9)][index % 9];
        if (cell_value !== 0) {
            cell.value = cell_value;
        } else {
            cell.value = '';
...
function check_board() {
    ...
    document.querySelectorAll('.sudoku-cell').forEach((cell, index) => {
        ...
        if (cell.value != cell_value) {
            console.log(`Cell ${index} is incorrect: ${cell.value} !== ${cell_value}`);
    ...
    if (allCorrect) {
        console.log("All cells are correct!");
        window.alert("Congratulations! All cells are correct!");
    } else {
        console.log("Some cells are incorrect.");
        window.alert("Some cells are incorrect.");
    }
}
